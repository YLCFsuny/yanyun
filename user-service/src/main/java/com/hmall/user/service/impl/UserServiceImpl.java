...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public UserLoginVO login(LoginFormDTO loginDTO) {
        ...
        // 2.根据用户名或手机号查询
        User user = lambdaQuery().eq(User::getUsername, username).one();  // 方法引用
        Assert.notNull(user, "用户名错误");  // 断言
        // 3.校验是否禁用
        ...
        ulvo.setUserId(user.getId());
        ulvo.setUsername(user.getUsername());
        ulvo.setBalance(user.getBalance());
        ulvo.setToken(token);
        return ulvo;
    }
